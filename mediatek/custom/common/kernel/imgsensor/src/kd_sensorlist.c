...
inline static int adopt_CAMERA_HW_Open(void)
{
	...
	if (atomic_read(&g_CamHWOpend) == 0) {
		...
		KD_IMGSENSOR_PROFILE("kdModulePowerOn"); 
                  printk("[adopt_CAMERA_HW_Open()]kdModulePowerOn\n");
		...
		KD_IMGSENSOR_PROFILE("SensorOpen"); 
                  printk("[adopt_CAMERA_HW_Open()]SensorOpen\n");
	}
	if (err == 0 ) {
...
********************************************************************************/
inline static int adopt_CAMERA_HW_CheckIsAlive(void)  //IICuX
{
	UINT32 err = 0;
